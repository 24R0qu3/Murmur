...
class SettingsDialog(tk.Toplevel):
    ...
    def _build(self):
        ...
        row(
            ...
            lambda r: ttk.Combobox(
                frm,
                textvariable=self._wakeword_var,
                values=_get_wakeword_models(),
                width=14,
            ).grid(row=r, column=1, sticky="w", **pad),
            5,
        )
        ...
        row(
            "Sensitivity",
            lambda r: ttk.Spinbox(
                frm,
                textvariable=self._wakeword_thresh_var,
                from_=0.10,
                to=1.00,
                increment=0.05,
                format="%.2f",
        ...
        tk.Checkbutton(
            frm,
            text="Bring to front on hotkey press",
            variable=self._raise_var,
        ).grid(row=9, column=0, columnspan=2, sticky="w", padx=12, pady=(0, 8))

        # Buttons
        btn_row = tk.Frame(frm)
        btn_row.grid(row=10, column=0, columnspan=3, pady=(4, 0))
        tk.Button(btn_row, text="Save", command=self._save, width=10).pack(
            side=tk.LEFT, padx=5
        )
        tk.Button(btn_row, text="Cancel", command=self.destroy, width=10).pack(
            side=tk.LEFT, padx=5
        )
        if self._on_recenter:
            tk.Button(
                btn_row, text="Recenter overlay", command=self._on_recenter, width=16
